const encode = (input)=>[
        ...input
            ...
            x.charCodeAt(0),
            i
        ]).sort().flatMap((x)=>x).join(".").match(/./g).flatMap((x, i)=>new Array(x == "." ? 1 : 2 + x * 2).fill((1 + i) % 2)).join("").replace(/(([01])\2*)/g, (x)=>`${+x ? "." : "-"}${x.length}`); /*Эта функция не является обратимой, потому что она теряет информацию при кодировании переменной. Каждому значению переменной может соответствовать несколько закодированных значений, поэтому невозможно однозначно восстановить исходную переменную из её закодированного значения.
